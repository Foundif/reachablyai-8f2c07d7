...
const ADDONS = [
  {
    ...
    desc: 'Connect one more WhatsApp Business number to your CRM',
    icon: Phone,
  },
  {
    id: 'contacts',
    name: 'Extra contacts',
    desc: 'Increase your contact limit beyond your plan',
    icon: Contact,
  },
  {
    id: 'clients',
    name: 'Extra clients',
    desc: 'Increase your client limit beyond your plan',
    icon: Users,
  },
];
...
const FAQS = [
  {
    ...
    a: 'Yes. Plus includes 1 WhatsApp Business number, Scale includes 3 and Supreme includes 10 — all managed from the same Reachably CRM. WhatsApp numbers are a separate limit from users, and extra numbers are available as an add-on.',
  },
  {
    q: 'How does the 50% first-month offer work?',
    a: 'Subscribe to a monthly plan before your trial ends and your first month is half price. From the second month onwards you pay the normal monthly price.',
  },
  {
    q: 'Are WhatsApp/Meta charges included?',
    a: 'No. WhatsApp/Meta conversation charges are separate and billed directly with Meta. Reachably only charges the platform subscription and optional add-ons.',
  },
  {
    q: 'Is there a free trial?',
    a: 'Every account gets a 7-day free trial with all features unlocked. No credit or debit card required to start.',
  },
  {
    q: 'Can I change plans later?',
    a: 'Yes — upgrade or downgrade at any time. Your data, WhatsApp connections, contacts and campaigns stay exactly as they are.',
  },
];
...
const PricingContent = () => {
  ...
  const [workspacePlanLoading, setWorkspacePlanLoading] = useState(!!user);

  const [scalePromptTrigger, setScalePromptTrigger] = useState(0);
  ...
  useEffect(() => {
    ...
    const loadWorkspacePlan = async () => {
      ...
      if (error) {
        ...
        setWorkspacePlanId(data?.plan_id || null);
      }
  ...
  const fallbackPlanId = PLANS.some((p) => p.id === currentStatus)
    ? currentStatus
    : null;

  const currentPlanId = workspacePlanId || fallbackPlanId;

  const activePlan =
    PLANS.find((p) => p.id === currentPlanId)?.id || null;

  const payableFor = (p: Plan) =>
    billing === 'yearly' ? p.yearly : p.promoMonthly;
  ...
  return (
    <div className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Manage your plan, usage, WhatsApp numbers and add-ons.
            </p>
          </div>
            ...
            {!workspacePlanLoading && !activePlan && (
              <div className="inline-flex flex-col items-center gap-1 px-5 py-3 rounded-2xl border border-primary/30 bg-primary/5 mb-6">
                ...
                <span className="text-sm font-medium">
                  Subscribe before your trial ends and enjoy 50% off your first month.
                </span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        <div
          id="plan-grid"
          className="grid gap-5 lg:grid-cols-3 scroll-mt-20"
        >
          {PLANS.map((p, i) => (
            <motion.div
              key={p.id}
                  ...
                  {billing === 'monthly' ? (
                    <>
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        Then {formatINR(p.monthly)}/month
                      </div>
                    </>
                  ) : (
                    <>
        ...
        {/* Scale upgrade prompt */}
        {activePlan === 'plus' && (
          <ScaleUpgradePrompt
            currentPlanId={activePlan}
            trigger={scalePromptTrigger}
            onUpgrade={() => {
              const scalePlan = PLANS.find(
                (plan) => plan.id === 'scale'
              );

              if (scalePlan) {
                void checkout(scalePlan);
              }
            }}
          />
        )}
        ...
        <div className="mt-14">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Need more than your plan includes? Add-ons are available on every plan.
          </p>
                ...
                <Button
                  ...
                  onClick={() =>
                    (window.location.href =
                      'mailto:foundifinnovations@gmail.com?subject=Reachably%20add-on%20request')
                  }
                >
                  Request add-on
                </Button>
        ...
        <Card className="mt-8 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              More than 25 users or 10 WhatsApp numbers? We'll build a plan around you.
            </p>
          </div>
          ...
          <Button
            ...
            onClick={() =>
              (window.location.href =
                'mailto:foundifinnovations@gmail.com?subject=Reachably%20Enterprise')
            }
          >
            Talk to sales
          </Button>
