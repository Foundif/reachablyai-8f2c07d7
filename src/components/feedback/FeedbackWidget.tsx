...
export default function FeedbackWidget() {
  ...
  const loadTickets = async () => {
    ...
    if (error) {
      return toast.error('Could not load your feedback');
    }
  ...
  const chooseFiles = (
    ...
    if (
      ...
      toast.error(
        'Use PNG, JPG, or WebP screenshots under 5 MB each'
      );
  ...
  const submit = async () => {
    if (
      ...
      return toast.error(
        'Add a title and at least 10 characters of detail'
      );
    }
    ...
    if (error) {
      if (error.code === '23505') {
        return toast.error(
          'Your current ticket must be cleared before submitting another'
        );
      }
  ...
  return (
    <>
      <Button
        ...
        className="fixed right-0 top-1/2 z-40 h-auto -translate-y-1/2 rounded-l-xl rounded-r-none px-2.5 py-3 [writing-mode:vertical-rl] shadow-lg md:px-3"
        aria-label="Open feedback"
      >
        <MessageSquarePlus className="mb-1 h-4 w-4 rotate-90" />
        Feedback
      </Button>
      ...
      >
        <DialogContent className="max-h-[90vh] w-[calc(100%-1.5rem)] overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Report a bug, request a feature, or share a
              small improvement.
            </DialogDescription>
          </DialogHeader>
          ...
          {tickets.filter(
            (ticket) => !ACTIVE.has(ticket.status)
          ).length > 0 && (
