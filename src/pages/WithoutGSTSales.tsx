...
import { FileBarChart, Search, Download, Eye, Trash2, MessageCircle, ChevronLeft, ChevronRight } from 'lucide-react';
import { toast } from 'sonner';
import { SalonInvoice } from '@/types/salon';

const PAGE_SIZE = 10;

const WithoutGSTSales = () => {
  ...
  const filtered = useMemo(() => {
    return nonGstInvoices.filter(i => {
  ...
  }, [nonGstInvoices, fromDate, toDate, searchQuery]);
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'non-gst-sales.csv'; a.click();
  };
    ...
    <AppLayout>
      <div className="p-4 md:p-6 lg:p-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-foreground">General Sales</h1>
              <p className="text-muted-foreground">{nonGstInvoices.length} invoices without GST</p>
            </div>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="glass-card p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-foreground mb-2">No General Sales</h3>
            <p className="text-muted-foreground">No invoices without GST found.</p>
          </div>
        ) : (
          <>
      ...
      <Dialog open={!!viewInvoice} onOpenChange={() => setViewInvoice(null)}>
        <DialogContent className="sm:max-w-lg bg-card border-border">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><FileBarChart className="w-5 h-5 text-primary" />Invoice Details</DialogTitle></DialogHeader>
          {viewInvoice && (
...
export default WithoutGSTSales;
